...
CustomTextRectangle::CustomTextRectangle(qreal x, qreal y, qreal width, qreal height, qreal borderSize, QBrush brush, app::AppState *appState, QString text)
{
    // set the member
...
void CustomTextRectangle::setTextItem(QGraphicsTextItem *textItem)
{
    this->m_textItem = textItem;
}

QGraphicsTextItem *CustomTextRectangle::getTextItem()
{
    return this->m_textItem;
}

void CustomTextRectangle::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    if(event->button() == Qt::LeftButton){
        switch(this->getAppState()->getSelectedTool()){
            ...
            default: {
                qWarning() << "Something went wrong - CustomTextRectangle/mousePressEvent";
                break;
            }
        }
    }
    CustomRectangle::mousePressEvent(event);
}

void CustomTextRectangle::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    CustomRectangle::mouseReleaseEvent(event);
}

int CustomTextRectangle::type() const
{
    return UserType+4;
}

CustomTextRectangle::~CustomTextRectangle()
{
